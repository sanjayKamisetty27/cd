...
export default function FoodStatistics() {
    ...
    const fetchFoodItems = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8081/fooddiary/list/${formattedDate}`,
          {
            headers: {
              email: email,
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8081/fooddiary/delete/${id}`, {
        headers: {
          email: email,
          Authorization: `Bearer ${authToken}`,
  ...
  const handleShowDetails = async (id) => {
    try {
      const response = await axios.get(
        `http://localhost:8081/fooddiary/details/${id}`,
        {
          headers: {
            email: email,
            Authorization: `Bearer ${authToken}`,
  ...
  const handleAddFood = async () => {
    ...
    if (!newFood.quantity || newFood.quantity <= 0) {
      setSnackbar({
        open: true,
        severity: "error",
    ...
    const adjustedFoodItem = {
      name: newFood.name,
      protein: newFood.protein * newFood.quantity,
      fat: newFood.fat * newFood.quantity,
      energy: newFood.energy * newFood.quantity,
      netCarbs: newFood.netCarbs * newFood.quantity,
      category: newFood.category,
      quantity: newFood.quantity, // Keep the quantity for reference
    };
  ...
  const handleClose = () => {
    setOpen(false);
    setNewFood({ name: "", protein: "", fat: "", energy: "", netCarbs: "", category: "", quantity: "" });
  };
    ...
    const fetchStats = async () => {
      try {
        ...
        const formattedDate = new Date(selectedDate.getTime() - selectedDate.getTimezoneOffset() * 60000)
          ...
          .split("T")[0];
        const response = await axios.get(
          `http://localhost:8081/fooddiary/stats/${formattedDate}`,
          { headers: { email } }
        );
  ...
  useEffect(() => {
    const fetchGraphData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8081/fooddiary/stats/graph/${formattedDate}`,
          { headers: { email } }
        );
  ...
  const getLineChartData = (nutrient) => {
            ...
            >
              {["energy", "protein", "fat", "netCarbs"].map((nutrient) => (
                ...
                >
                  ...
                  <Line
                    data={getLineChartData(nutrient)}
                    options={{
                      responsive: true,
                      plugins: {
                        legend: {
            ...
            >
              ...
              >
                <Box sx={{ position: "relative", display: "inline-block" }}>
                  ...
                  >
                    <FontAwesomeIcon icon={faBacon} style={{ color: "#f44336", fontSize: "30px" }} />
                  </IconButton>
                </Box>
              ...
              <>
                ...
                >
                  <Typography sx={{ marginBottom: 1 }}>
                    <strong>Energy:</strong> {popupDetails.energy || 'N/A'} kcal
                  </Typography>
                  <Typography sx={{ marginBottom: 1 }}>
                    <strong>Protein:</strong> {popupDetails.protein || 'N/A'} g
                  </Typography>
                  <Typography sx={{ marginBottom: 1 }}>
                    <strong>Fat:</strong> {popupDetails.fat || 'N/A'} g
                  </Typography>
                  <Typography sx={{ marginBottom: 1 }}>
                    <strong>Net Carbs:</strong> {popupDetails.netCarbs || 'N/A'} g
                  </Typography>
                  <Typography sx={{ marginBottom: 1 }}>
                    <strong>Category:</strong> {popupDetails.category || 'N/A'}
                  </Typography>
                  <Typography sx={{ marginBottom: 2 }}>
                    <strong>Quantity:</strong> {popupDetails.quantity || 'N/A'}
                  </Typography>
                </Box>
